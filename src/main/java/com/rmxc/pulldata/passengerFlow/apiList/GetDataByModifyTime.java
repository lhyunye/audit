 ...
 */
@Slf4j
public class GetDataByModifyTime extends ApiModel {

    private static String apiPath = "/huikeapi/passengerFlowData/V3/getDataByModifyTime";
    ...
    //获取当前时间的前一天
    private static String modifyTime = LocalDateTime.now().format(df_zero);
    ...
    public void exec(String date) {
        ...
        if (ObjectUtils.isEmpty(date)){
            paramsMap.put("modifyTime", this.modifyTime);
        }else {
            paramsMap.put("modifyTime", LocalDate.parse(date,df_day2).format(df_zero));
        }
